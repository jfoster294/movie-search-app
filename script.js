...
function renderHero() {
  ...
  if (!selectedMovie) {
    ...
    heroPoster.innerHTML = `
      <span>LOCAL</span>
      <strong>SCAN YOUR MOVIE FOLDER</strong>
    `;

    heroTitle.textContent = "Build your local cinema library.";
    heroDescription.textContent =
      "Use the Scan Movie Folder button at the top to select a folder from your computer or drive. Cinema Library will detect your real movie files, estimate storage, and create retro-styled thumbnails when possible.";
...
function renderPoster(container, movie) {
  const isHeroPoster = container.id === "heroPoster";
  const posterClass = isHeroPoster ? "hero-poster" : "details-poster";

  container.className = `${posterClass} ${movie.poster ? "" : "fallback-poster"}`;
